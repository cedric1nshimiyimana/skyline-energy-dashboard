# skyline_aether_phase2_dynamic_final.py — FINAL FIX: Vectorized Data + Pandas Styler Correction
...
def generate_history_data():
    ...
    # 3. FIX (Vectorized Grid_KWH Calculation)
...
def generate_live_data():
    ...
    # Battery logic
    if solar_kw > load_kw + 1: battery_kw = round(random.uniform(1.0, 4.0), 2) * -1
    elif load_kw > solar_kw + 1: battery_kw = round(random.uniform(1.0, 3.5), 2)
    else: battery_kw = round(random.uniform(-0.5, 0.5), 2)
        
    grid_kw = round(max(0.0, load_kw - solar_kw - battery_kw), 2)
    
    # State-dependent SOC (mock)
    if 'battery_soc' not in st.session_state: st.session_state.battery_soc = random.randint(50, 95)
    
    if battery_kw < -0.1: st.session_state.battery_soc = min(100, st.session_state.battery_soc + 1)
    elif battery_kw > 0.1: st.session_state.battery_soc = max(10, st.session_state.battery_soc - 1)
    ...
    st.session_state.today_kwh += solar_kw * 0.05
...
def load_custom_css():
    ...
    /* FIX: Custom styling for the dataframe highlight */
    .stDataFrame table td.highlighted {
...
def render_dynamic_flow(data):
    ...
    grid_export = solar_kw > load_kw + abs(battery_kw) + 0.1 and abs(grid_kw) > 0.1
    ...
    with flow_cols[1]:
        st.markdown(f"<span class='flow-arrow' style='color:#00ffff;' title='Solar Generation'>{solar_kw:.1f} kW →</span>", unsafe_allow_html=True)

    # Central Hub
    with flow_cols[2]:
        load_battery_cols = st.columns([1, 1, 1])
        
        with load_battery_cols[0]: # Grid Status
            grid_color = "#FF0000" if grid_import else ("#FFD700" if grid_export else "#88ffff")
            st.markdown(f"<p style='text-align:center; color:{grid_color};'>**Grid** ({abs(grid_kw):.1f} kW)</p>", unsafe_allow_html=True)

        with load_battery_cols[1]: # Load Icon
            st.markdown(f"<p style='font-size:5rem; text-align:center;'>🏠</p>", unsafe_allow_html=True)
        
        with load_battery_cols[2]: # Battery Status
        ...
        # Arrows BELOW: Primary Load Source
        st.markdown("<div style='display:flex; justify-content:space-around;'>", unsafe_allow_html=True)
        
        if grid_import:
            flow_desc, flow_style = f"Grid → Load ({load_kw:.1f} kW)", "#FF0000"
        elif battery_kw > 0.1:
            flow_desc, flow_style = f"Battery → Load ({load_kw:.1f} kW)", "#ff9f1c"
        elif solar_to_load:
             flow_desc, flow_style = f"Solar → Load ({load_kw:.1f} kW)", "#00ffff"
        else:
            flow_desc, flow_style = "Load Idle", "#88ffff"
            
        st.markdown(f"<span class='flow-arrow' style='font-size:2rem; color:{flow_style};' title='Primary Load Source'>{flow_desc}</span>", unsafe_allow_html=True)
...
def page_history(df):
    ...
    fig.update_layout(
        plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', font_color="#88ffff", height=500,
        xaxis_title="Date", yaxis_title="Energy (kWh)", xaxis=dict(showgrid=False),
        yaxis=dict(gridcolor='rgba(136, 255, 255, 0.2)'), hovermode="x unified",
...
def page_aether_coins(data):
    ...
    st.markdown("Aether Coins are rewarded for self-sufficiency and high solar production.")
    
    st.subheader(f"Your Balance: **{data['solar_coins']:.2f} AETHER** 🪙")
    
    col_buy, col_stats = st.columns(2)
    
    # 1. THE STYLING FUNCTION FIX
    def highlight_you(s):
        """Highlights the row where the 'User' column value is 'You'."""
        # FIX: Directly check the values in the 'User' column series
        is_you = s == 'You'
    ...
    with col_stats:
        ...
        # 2. APPLY STYLING CORRECTLY: Apply the function only to the 'User' column
        styled_df = ranking_df.sort_values('Rank').set_index('Rank').style.apply(
            highlight_you, subset=['User'], axis=0 
        )
