# skyline_aether_phase2_1_control.py — Phase 2.1 Ready (Two-Way Control)
# This version adds secure MQTT command publishing from the 'Remote Control' tab.
...
import io
import threading
import json # Added for publishing structured data
...
st.set_page_config(page_title="Skyline Aether — Phase 2.1 Control", layout="wide", initial_sidebar_state="expanded")
...
if not os.path.exists('style.css'):
    try:
        with open('style.css', 'w') as f:
            f.write(DEFAULT_CSS)
        # print('style.css created with default theme — modify as needed.')
    except Exception:
        # print('Could not write style.css:', e)
        pass
...
# -------------------- MQTT Background Worker & Command Publisher (NEW) --------------------
...
def send_mqtt_command(site_id, control_path, value):
    ...
    Example topic: cerbo/KIG-001/settings/min_soc
    """
    if not st.session_state.mqtt_started or 'mqtt_client' not in st.session_state:
        st.error("MQTT Client is not active. Command not sent.")
        return False
    ...
    try:
        ...
        # Payload must often be a JSON structure for Victron control, e.g., {"value": 20}
...
with st.sidebar:
    ...
    if not st.session_state.mqtt_started:
        st.info('MQTT not started. To enable live data, set CERBO_MQTT_HOST env var and ensure paho-mqtt installed.')
...
if use_mqtt:
    ...
    # If using MQTT, ensure SOC is float/int
...
# -------------------- Main Tabs --------------------
tab_overview, tab_analytics, tab_reports, tab_control = st.tabs(["Overview", "Analytics", "Reporting", "Remote Control"])
    ...
    with tab_control:
        if st.session_state.user_role in ['Admin','Integrator']:
            ...
            with c1:
                ...
                # Command 1: Reboot
                if st.button('REBOOT INVERTER (Critical)', type="primary"):
                    # Victron uses a topic for system reset
                    send_mqtt_command(current_site_id, 'system/reboot', 1) 
                
                # Command 2: Force Grid Charge
                if st.button('FORCE GRID CHARGE (Maintenance)'):
                    # Mock a different control path/value
            ...
            with c2:
                ...
                if st.button('APPLY PARAMETERS', type="secondary"):
                    # This sends the min_soc setting
...
# -------------------- Footer --------------------
st.markdown('---')
st.caption('Skyline Aether — Phase 2.1 Control Ready • This file contains the architecture for a multi-million dollar valuation.')